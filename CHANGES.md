 ...
 * Bugfix: TickStore read produced garbage far-future timestamps (e.g. year 2124/2262) and a non-monotonic "data is out of order" index for reads spanning the v3→v4 bucket boundary. v3 buckets decode the index to uint64, v4 (varint) buckets to int64; `np.concatenate` of the mixed list has no common integer dtype and upcasts to float64, whose bit pattern the subsequent `.view(np.int64)` then misreads. `_index_ms_to_ns` now views each array to int64 before concatenating. Pure-v3 or pure-v4 reads were unaffected.
